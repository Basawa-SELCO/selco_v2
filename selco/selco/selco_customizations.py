...
from frappe.utils import flt,now,now_datetime, get_link_to_form, round_based_on_smallest_currency_fraction
import operator
...
def autocreate_service_record(doc):
	if doc.workflow_state == "Complaint Assigned To CSE" and not frappe.db.exists("Service Record",{'selco_complaint_number':doc.name,'auto_created': 1,'docstatus':['!=',2]}):
		...
		service_record.selco_customer_full_name = doc.selco_customer_full_name
		service_record.selco_customer_contact_number = doc.selco_customer_contact_number
		service_record.selco_landline_mobile_2 = doc.selco_customer_contact_number_landline
		service_record.selco_detail_address = doc.selco_detail_address
		service_record.selco_add_address = doc.selco_customer_address
		service_record.selco_complaint_date = doc.selco_complaint_received_date
		service_record.selco_cse_name = doc.cse_name
		#service_record.service_area = doc.selco_service_area
		service_record.selco_sales_person = doc.selco_sales_person
	...
	if doc.is_return:
		if doc.return_against:
			for d in frappe.get_all('Maintenance Schedule',
				filters={'sales_invoice': doc.return_against, 'docstatus': 1}):
...
def make_maintenance_visit(doc):
	for d in doc.schedules:
		mv_doc = frappe.new_doc("Maintenance Visit")
		mv_doc.company = doc.company
		mv_doc.branch = doc.selco_branch
		mv_doc.selco_branch = doc.selco_branch
		mv_doc.customer = doc.customer
		mv_doc.customer_name = doc.customer_name
		mv_doc.customer_group = frappe.db.get_value("Customer",doc.customer,'customer_group')
		mv_doc.territory = frappe.db.get_value("Customer",doc.customer,'territory')
		mv_doc.customer_landline_number = doc.selco_landline_mobile_2
		mv_doc.customer_contact_number = doc.selco_customer_contact_number
		mv_doc.customer_address = doc.customer_address
		mv_doc.address_display = doc.address_display
		mv_doc.contact_person = doc.contact_person
