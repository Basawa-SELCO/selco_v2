...
def get_sr_details(filters):
	...
	return frappe.db.sql("""SELECT A.name,A.selco_complaint_received_date,A.selco_customer_full_name,A.selco_ics_date,A.selco_remarks,B.selco_service_record_number_1,B.selco_service_person,B.selco_service_record_date_1,B.selco_service_amount,B.within_warranty,B.selco_special_budget_approved_by_csd,B.budget_thirty_five,B.selco_csd_remarks FROM `tabIssue` AS A INNER JOIN `tabService Record Details Issue` AS B ON A.name=B.parent WHERE selco_service_branch_email_id = %s AND B.selco_service_record_date_1 BETWEEN %s AND %s""", (selco_branch,msd,med),as_dict=1)
