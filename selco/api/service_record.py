...
# 		parent_dict['fault_rectified_and_replacement_details'] = frappe.db.get_values("Service Record Item Details",{'parenttype':'Service Record','parent':row.name},child_fields, as_dict=True)
...
def update_service_record():
	if frappe.request.data:
		...
		parent_field_list = ['selco_customer_feedback','selco_service_charges_collected','selco_customer_remarks','selco_customer_signature','selco_cse_location','selco_cse_feedback','selco_job_status','selco_cse_name','selco_cse_signature','selco_customer_id','selco_cse_remarks','selco_complaint_number','selco_cse_date','submitted']
...
def update_child_records(request_data, doc):
	if request_data.get("fault_rectified_and_replacement_details"):
		child_field_list = ['selco_within_warranty','selco_serial_number','selco_collected_amount','selco_make','selco_remarks']
		for row in request_data.get("fault_rectified_and_replacement_details"):
			if not row.get('name'):
				frappe.throw('Define name in fault_rectified_and_replacement_details to update child record.')
			...
			for child_field in child_field_list:
				if row.get(child_field):
					for item in doc.fault_rectified_and_replacement_details:
